...
def read_parquet_tables(data_dir: Path) -> dict[str, list[dict[str, Any]]]:
    try:
        ...
        raise RuntimeError(
            "Parquet input needs pandas and pyarrow. Install them with "
            "`python -m pip install pandas pyarrow`, or use the included Markdown exports."
        ) from exc
    tables = {}
...
def load_tables(data_dir: Path) -> dict[str, list[dict[str, Any]]]:
    if all((data_dir / f"{name}.md").is_file() for name in REQUIRED):
        ...
        missing = [
            f"{data_dir / (name + '.md')} or {data_dir / (name + '.parquet')}"
            for name in REQUIRED
            if not (data_dir / f"{name}.md").is_file() and not (data_dir / f"{name}.parquet").is_file()
        ]
        raise FileNotFoundError("Missing input table(s): " + ", ".join(missing))
    for name, required in REQUIRED.items():
        if not tables[name]:
            raise ValueError(f"{name} table is empty")
        actual = set(tables[name][0])
        absent = set(required) - actual
        if absent:
            raise ValueError(f"{name} is missing required fields: {', '.join(sorted(absent))}")
    return tables
...
def normalize_tables(tables: dict[str, list[dict[str, Any]]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    nodes = [
        {"gid": int(row["gid"]), "depth": int(row["depth"]), "is_seed": as_bool(row["is_seed"])}
...
def validate_inputs(nodes: list[dict[str, Any]], edges: list[dict[str, Any]], tx: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    agg: dict[tuple[int, int], list[float]] = defaultdict(lambda: [0.0, 0.0])
    for row in tx:
        if not math.isfinite(row["sum_kzt"]) or row["sum_kzt"] <= 0:
            raise ValueError(f"Transaction amount must be finite and positive: {row}")
        key = (row["src"], row["dst"])
...
def main() -> int:
    ...
    (args.out / "index.html").write_text(dashboard, encoding="utf-8")
    ...
    (args.out / "analysis_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
